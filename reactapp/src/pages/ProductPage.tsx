...
function ProductPage() {
	...
	const fetchProductAndReviews = useCallback(async () => {
		try {
			...
			const productData = productResponse.data.products.find(
			...
			const totalRating = reviewsData.reduce(
				(acc: number, review: Reviews) => acc + review.rate,
				0
			)
			setAverageRating(
	...
	const countReviewsByRate = useCallback(
		(rate: number): number =>
			reviews.filter(review => review.rate === rate).length,
		[reviews]
	)

	const percentageOfRate = useCallback(
		(rate: number): number => (countReviewsByRate(rate) / reviews.length) * 100,
		[countReviewsByRate, reviews.length]
	)
				...
				<div className='product-page__info-div'>
					...
					<div className='product-page__container-div'>
						...
						<div className='product-page__price-buy-div'>
							...
							<Link
								to={`/payment?price=${product.price}`}
								className='product-page__buy-link'
							>
								Купить
							</Link>
						</div>
			...
			<section className='product-page__reviews-section'>
				...
				<div className='reviews-section__rate-block'>
					<div className='rate-block__rating'>
						...
						<div className='rate-block__star-rating'>
							<div className='star-rating__back-stars'>
								...
								<div
									className='star-rating__front-stars'
					...
					<div className='rate-block__progressbars-group'>
						{[5, 4, 3, 2, 1].map(rate => (
							<div
								className='progressbars-group__progressbar-container'
								key={rate}
							>
								<span className='rate-progressbar__rate-number'>{rate}</span>
								<div className='progressbar-container__progressbar'>
									<div
										className='progressbar__active-line'
										style={{ width: `${percentageOfRate(rate)}%` }}
									></div>
								</div>
							</div>
						))}
